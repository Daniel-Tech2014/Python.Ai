...
def list_genres(df):
    return sorted(set(genre for sublist in df['Genre'].dropna().str.split(','))for genre in sublist)

genres=list_genres(movies_df)
# Recommend movies based on filters (genre, mood, rating)
def recommend_movies(df, genre=None, mood=None, min_rating=0, num_recommendations=5):
    filtered_df = movies df
    if genre:
        ...
        if rating:
            ...
            filtered_df = filtered_df.resest_index(drop=True)
